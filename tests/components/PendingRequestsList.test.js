...
describe('PendingRequestsList', () => {
  ...
  it('handles accept request', async () => {
    const { findAllByText, queryByText, findByText } = render(<PendingRequestsList />);
  ...
  it('handles decline request', async () => {
    ...
    await findByText('user1');
    const declineButtons = await findAllByText('Rechazar');
  ...
  it('handles fetch requests error', async () => {
    ...
    // The component will fail to fetch initially but shouldn't crash
    await waitFor(() => {
      expect(friendsApi.getPendingRequests).toHaveBeenCalledWith(1);
    });
    
    // Shouldn't show the items since they didn't load
    expect(queryByText('user1')).toBeNull();
    spy.mockRestore();
  });
  ...
  it('handles refresh', async () => {
    ...
    expect(queryByText('user1')).toBeNull(); // It was replaced by refresh
  });
